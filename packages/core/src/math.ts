...
export function clipOutliersByWinsorization(values: number[], lowerPct: number, upperPct: number): number[] {
  ...
  const lo = at(lowerPct); const hi = at(upperPct);
  return values.map((v) => Number.isFinite(v) ? Math.max(lo, Math.min(hi, v)) : v);
}
export function mean(values: number[]): number { const clean = values.filter(Number.isFinite); return clean.length ? clean.reduce((a,b)=>a+b,0)/clean.length : 0; }
